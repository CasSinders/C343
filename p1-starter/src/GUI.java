...
public class GUI extends JFrame {
	...
	// bitSelector is the interface element for selecting desired number of bits
	// per channel.
	...
	// colorBox is the interface element for displaying the most recently
	// selected color
	...
	/**
	 * Constructs a window to display two images and compare them for similarity
	 * after quantization.
	 */
	public GUI(Image leftImage, Image rightImage) {
		...
		// sideBySide is the interface element for displaying the two images
		// side by side.
		...
		JPanel sideBySide = new JPanel() {
			{
				...
				setOpaque(true);
				add(A);
				add(Box.createRigidArea(new Dimension(10, 0))); // put some
																// space between
																// the images
				add(B);
			}
	...
	/**
	 * Returns the similarity of the two images using however many bits per
	 * channel appear on bitSelector. Results are memoized to speed the process.
	 */
	...
	/**
	 * An ImagePanel represents one panel displaying one image. The image is
	 * quantized depending on the bitSelector value. This component listens for
	 * mouse events, and updates the colorBox control accordingly.
	 */
	class ImagePanel extends JPanel {
		...
		public ImagePanel(Image image) {
			// Effects on one image affect both, so save all constructed panels
			// in one place.
			panels.add(this);
			setBackground(Color.WHITE);
			this.image = image;
			...
			addMouseListener(new MouseAdapter() {
				public void mousePressed(MouseEvent e) {
					...
					timer = new Timer();
					// Schedule the xray effect to turn on 100ms after the
					// intial press.
	...
	/**
	 * Returns a black and white version of the given image, where pixels
	 * matching color are white.
	 */
